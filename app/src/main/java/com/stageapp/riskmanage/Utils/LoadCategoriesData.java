...
public class LoadCategoriesData {
    ...
    public LoadCategoriesData(Context context) {
...
//                        }
        ...
        if (appsList.size() > 0) {
            for (int i= 0;i < 8;i++) {
                final String str = ApplicationInfo.getCategoryTitle(this.context,i).toString();
                List<App> mlist = appsList.stream().filter(app -> app.categoryTitle.contains(str)).collect(Collectors.toList());
                long duration = mlist.stream().map(App::getUsageDuration).mapToLong(Long::longValue).sum();
                int percnt = (int) (duration* 100 / totalTime );
                if (duration > 0){
                    Log.e("Stats",str+": "+duration);
                    myCategorie.add(new Categorie(i,str,percnt,duration,0));
                }

            }
        }

        Collections.sort(myCategorie, Comparator.comparingLong(Categorie::getTimeSpentDay));
        Collections.reverse(myCategorie);
        if (!weekStats) {
            Paper.book().write("CategoriesDay", myCategorie);
            Paper.book().write("DayUsageStats", new DayUsageStats(date, (totalTime), 0));
        }else {
            Paper.book().write("CategoriesWeek", myCategorie);
